...
object Day11bMut:
  ...
  class Monkey(num: Int,
    val items: Queue[BigInt],
    val op: BigInt => BigInt,
    val predicate: BigInt => Boolean,
    val divBy: Long,
    val trueMonkey: Int,
    val falseMonkey: Int,
  ...
  object Monkey:
    def fromList(params: List[String]): Monkey = params match
      case num :: starts :: op :: oparg :: divarg :: truemonkey :: falsemonkey :: Nil => 
        val items = starts.split(", ?").map(BigInt.apply).to(Queue)
        val opfun: BigInt => BigInt = (op, oparg) match
          case ("*", s) if s == "old"   => n => n * n
          case ("*", s)                 => { val b = BigInt(s); n => n * b }
          case ("+", s) if s == "old"   => n => n + n
          case ("+", s)                 => { val b = BigInt(s); n => n + b }
        val divBy = divarg.toLong
        val pred: BigInt => Boolean = n => n % divBy == 0
