...
object Day7b:
  ...
  def main(args: Array[String]): Unit =
    ...
    val minSize =   30000000 - (70000000 - root.calculateTotalSize)
    println(s"min size: $minSize")
    val filtered = root.findAll(_.calculateTotalSize >= minSize)
      .map(dir => (dir.name, dir.calculateTotalSize)).sortBy(_._2)
    println(s"found ${filtered.length} matching dirs")
    filtered.foreach(println)
    val smallest = filtered.head
    val largest = filtered.last
    println(s"The smallest that will free sufficient space is: $smallest")
    println(s"The largest that will free sufficient space is: $largest")
  ...
  class Dir(val name: String, val parent: Option[Dir]):
    ...
    override def toString(): String = s"($name, $parent, $files, ${dirs.size} subdirs)"
